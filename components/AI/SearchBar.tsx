...
const chooseModel = (id: string) => {
  ...
  const handleModeClick = (m: "act" | "chat") => {
    onModeChange?.(m);
      ...
      <div className="rounded-xl bg-white border border-gray-200  shadow-lg p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {quickActions.map((a) => (
                  <button
                    ...
                    className="text-xs px-3 py-1 rounded-full border hover:bg-gray-100 dark:hover:bg-gray-900 transition text-gray-700 dark:text-gray-200 bg-white/50 dark:bg-gray-900/40"
                  >
                    {a.label}
                  </button>
                ))}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <textarea
                ...
                onKeyDown={onKeyDown}
                placeholder="Ask Galuxium AI anything — web, images, code, summaries..."
                disabled={disabled}
                className="w-full resize-none bg-transparent p-3 text-md placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none min-h-[56px] max-h-[40vh] overflow-auto"
                style={{ height: 40 }}
              />
        ...
        <div className="flex flex-row items-center justify-between">
          ...
          <div className="text-gray-500 flex flex-row items-center gap-3">
            ...
            <div onClick={() => setOpen(!open)} className="text-xs flex cursor-pointer items-center gap-2 font-medium text-white h-fit py-2 px-3 rounded-md bg-gradient-to-r from-[#2000c1] to-[#2e147e] select-none">
              <Cpu className="size-4" />
              <span className="truncate max-w-[10rem]">
                {models.find((m) => m.id === selectedModel)?.name || (models[0]?.name ?? "Select model")}
              </span>
            </div>


            {open && (
              <div className="absolute  text-xs md:text-1.5xl md:w-[15vw] mb-55 rounded-xl shadow-lg overflow-hidden bg-gradient-to-r from-[#2000c1] to-[#2e147e] text-white z-10">
                {models.length === 0 ? (
                  ...
                  models.map((m) => (
                    <div
                      ...
                      className={`px-4 py-2 cursor-pointer hover:bg-[#2e147e]/80 transition-all duration-200 ${selectedModel === m.id ? "bg-white/20" : ""}`}
                    >
            ...
            className="flex items-center justify-center w-8 h-7 rounded-md bg-[#2e147e] text-white hover:scale-95 transition disabled:opacity-50"
            title="Send"
          >
            <IconSend/>
          </button>
